...
class ReducedDatasetBuilder:
    ...
    def saveMinLabelSet(self):
        with open(f'{self.baseDir}/ReducedLabelSet.dat', 'wb') as fd:
            pickle.dump(self.minLabelSet, fd)
        with open(f'{self.baseDir}/ReducedLabelSetIx2Label.dat', 'wb') as fd:
            pickle.dump(self.minLabelSetIx2Label, fd)
        with open(f'{self.baseDir}/ReducedLabelSetLabel2Ix.dat', 'wb') as fd:
            pickle.dump(self.minLabelSetLabel2Ix, fd)

    def saveReducedFileList(self):
        fd = open(f'{self.baseDir}/ReducedFileIndex.txt', 'w')
...
if __name__ == '__main__':
    print('Init')
    rdb = ReducedDatasetBuilder('.','FilesIndex.txt')
    print('Done!')
